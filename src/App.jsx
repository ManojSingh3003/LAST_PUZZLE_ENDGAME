...
export default function App() {
  const { grid, playerPos, gameWon, drills, mode, switchMode,uniqueSteps,par,timeElapsed } = useGameLogic();
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen text-white bg-gray-950 p-4">
      ...
      <div className="flex flex-wrap justify-center gap-6 mb-6 text-xl bg-gray-900/50 p-4 rounded-lg border border-gray-700 w-full max-w-2xl">
        ...
        <div className="text-gray-400 flex items-center">
          Unique Steps: <span className="text-white font-bold text-2xl ml-2">{uniqueSteps}</span>
          <span className="text-gray-600 text-sm ml-2 self-end mb-1">/ {targetMoves}</span>
        </div>
      ...
      {gameWon && (
        <div className="mb-6 p-4 bg-green-900/50 border border-green-500 text-green-300 rounded-lg text-center animate-bounce">
             ...
             <div className="text-white mt-2">
               {uniqueSteps <= targetMoves ? "PERFECT RUN!🏆" : "Can you do it faster?"}
             </div>
             <div className="text-yellow-300 text-sm mt-1">
               Time: {formatTime(timeElapsed)}s
             </div>
        </div>
      )}
